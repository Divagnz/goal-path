...
async def create_task_modal(request: Request, project_id: str = None, parent_task_id: str = None, db: Session = Depends(get_db)):
    """Create task modal with optional project and parent task pre-selection"""
    projects = db.query(Project).filter(Project.status == "active").all()
    parent_task = db.query(Task).filter(Task.id == parent_task_id).first() if parent_task_id else None
...
async def task_detail(task_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    subtasks = db.query(Task).filter(Task.parent_task_id == task_id).order_by(Task.created_at.desc()).all()
    
    # Get parent task if this is a subtask
    parent_task = db.query(Task).filter(Task.id == task.parent_task_id).first() if task.parent_task_id else None
    ...
    context = {
        ...
        "completion_stats": {
            ...
            "completion_percentage": (len([st for st in subtasks if st.status == "done"]) / len(subtasks) * 100) if subtasks else 0
        }
    }
