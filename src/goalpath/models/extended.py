...
class Reminder(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('pending', 'acknowledged', 'snoozed', 'cancelled')", name="chk_reminder_status"),
        CheckConstraint("(task_id IS NOT NULL AND project_id IS NULL) OR (task_id IS NULL AND project_id IS NOT NULL)", name="chk_reminder_relation"),
    )
...
class Issue(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("issue_type IN ('bug', 'feature', 'enhancement', 'question')", name="chk_issue_type"),
        CheckConstraint("priority IN ('low', 'medium', 'high', 'critical')", name="chk_issue_priority"),
        CheckConstraint("status IN ('triage', 'backlog', 'in_progress', 'resolved', 'closed')", name="chk_issue_status"),
    )
...
class TaskComment(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("comment_type IN ('comment', 'status_change', 'assignment', 'attachment')", name="chk_comment_type"),
    )
...
class ProjectContext(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("context_type IN ('notes', 'links', 'files', 'settings', 'metadata')", name="chk_context_type"),
...
class ScheduleEvent(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("event_type IN ('deadline', 'meeting', 'milestone', 'reminder')", name="chk_event_type"),
        CheckConstraint("end_datetime IS NULL OR end_datetime > start_datetime", name="chk_event_dates"),
    )

# Export all models
__all__ = [
    "Base",
    "Project", "Task", "TaskDependency", 
    "Goal", "GoalProject",
    "Sprint", "SprintTask",
