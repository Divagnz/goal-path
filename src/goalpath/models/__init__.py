...
class Project(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("status IN ('active', 'paused', 'completed', 'archived')", name="chk_project_status"),
        CheckConstraint("priority IN ('low', 'medium', 'high', 'critical')", name="chk_project_priority"),
        CheckConstraint("target_end_date IS NULL OR start_date IS NULL OR target_end_date >= start_date", name="chk_project_dates"),
        CheckConstraint("actual_end_date IS NULL OR start_date IS NULL OR actual_end_date >= start_date", name="chk_actual_end_date"),
    )
...
class Task(Base):
    ...
    dependencies_as_task = relationship("TaskDependency", foreign_keys="TaskDependency.task_id", back_populates="task")
    dependencies_as_dependency = relationship("TaskDependency", foreign_keys="TaskDependency.depends_on_task_id", back_populates="depends_on_task")
    sprint_links = relationship("SprintTask", back_populates="task")
    
    # Constraints
    __table_args__ = (
        CheckConstraint("task_type IN ('epic', 'story', 'task', 'subtask', 'milestone', 'bug')", name="chk_task_type"),
        CheckConstraint("status IN ('backlog', 'todo', 'in_progress', 'in_review', 'done', 'blocked', 'cancelled')", name="chk_task_status"),
        CheckConstraint("priority IN ('lowest', 'low', 'medium', 'high', 'highest', 'critical')", name="chk_task_priority"),
        CheckConstraint("due_date IS NULL OR start_date IS NULL OR due_date >= start_date", name="chk_task_dates"),
        CheckConstraint("story_points IS NULL OR story_points > 0", name="chk_story_points"),
        CheckConstraint("estimated_hours IS NULL OR estimated_hours >= 0", name="chk_estimated_hours"),
...
class TaskDependency(Base):
    ...
    depends_on_task = relationship("Task", foreign_keys=[depends_on_task_id], back_populates="dependencies_as_dependency")
    
    # Constraints
    __table_args__ = (
        CheckConstraint("dependency_type IN ('blocks', 'subtask_of', 'related_to')", name="chk_dependency_type"),
...
class Goal(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("goal_type IN ('long_term', 'medium_term', 'short_term', 'milestone')", name="chk_goal_type"),
        CheckConstraint("status IN ('active', 'paused', 'completed', 'cancelled')", name="chk_goal_status"),
        CheckConstraint("progress_percentage >= 0 AND progress_percentage <= 100", name="chk_progress_percentage"),
    )
...
class Sprint(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("status IN ('planning', 'active', 'completed', 'cancelled')", name="chk_sprint_status"),
