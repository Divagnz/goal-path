        ...
        if search:
            ...
            tasks_data = [
                ...
                if search_lower in task["title"].lower() or 
                   search_lower in (task["description"] or "").lower()
            ]
        
        # Add dependency count for each task
        for task_data in tasks_data:
            from ..models import TaskDependency
            dependency_count = db.query(TaskDependency).filter(
                TaskDependency.task_id == task_data["id"]
            ).count()
    ...
    try:
        with TransactionManager(db) as db_session:
            ...
            if task.parent_task_id:
                ...
                if not QueryUtils.validate_task_hierarchy(db_session, "new_task", task.parent_task_id):
                    raise HTTPException(
                        status_code=400,
                        detail="Invalid task hierarchy - would create a cycle"
                    )
    ...
    **Database Implementation**: Updates task in database with validation.
    ...
    try:
        with TransactionManager(db) as db_session:
            ...
            if "parent_task_id" in update_data:
                ...
                if new_parent_id:
                    ...
                    if not parent_task:
                        raise HTTPException(
                            status_code=404,
                            detail=f"Parent task with ID {new_parent_id} not found"
                        )
            ...
            from ..models import TaskDependency
            dependency_count = db_session.query(TaskDependency).filter(
                TaskDependency.task_id == task_id
            ).count()
...
async def delete_task(
    task_id: str,
    cascade: bool = Query(False, description="Delete subtasks (true) or reassign to parent (false)"),
    db: Session = Depends(get_db)
) -> MessageResponse:
    """
    Delete a task.
    ...
    try:
        with TransactionManager(db) as db_session:
            ...
            return MessageResponse(
                message=f"Task '{task_title}' {action_desc}deleted successfully"
            )
...
async def get_task_subtasks(
    ...
    **Database Implementation**: Retrieves subtasks with hierarchy.
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving subtasks: {str(e)}"
        )
...
async def get_task_dependencies(
    ...
    **Database Implementation**: Retrieves task dependency relationships.
    ...
    try:
        ...
        depends_on = db.query(TaskDependency, Task).join(
        ...
        dependents = db.query(TaskDependency, Task).join(
            Task, TaskDependency.task_id == Task.id
        ).filter(TaskDependency.depends_on_task_id == task_id).all()
