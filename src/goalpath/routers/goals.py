...
async def get_goal(goal_id: str, db: Session = Depends(get_db)) -> GoalResponse:
    """
...
async def create_goal(goal: GoalCreate, db: Session = Depends(get_db)) -> GoalResponse:
    """
    Create a new goal.
    ...
    **Database Implementation**: Creates goal in database with hierarchy validation.
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Goal creation failed: {str(e)}")
    except Exception as e:
...
async def update_goal(
    goal_id: str, goal_update: GoalUpdate, db: Session = Depends(get_db)
) -> GoalResponse:
    """
    ...
    **Database Implementation**: Updates goal with progress recalculation.
    ...
    try:
        with TransactionManager(db) as db_session:
            ...
            if "parent_goal_id" in update_data:
                ...
                if new_parent_id:
                    ...
                    if not parent_goal:
                        raise HTTPException(
                            status_code=404, detail=f"Parent goal with ID {new_parent_id} not found"
                        )
            ...
            project_links = (
                db_session.query(GoalProject).filter(GoalProject.goal_id == goal_id).all()
            )
        ...
        raise HTTPException(status_code=400, detail=f"Goal update failed: {str(e)}")
    except Exception as e:
    ...
    **Database Implementation**: Deletes goal with subgoal handling and project link cleanup.
    ...
    try:
        with TransactionManager(db) as db_session:
            ...
            return MessageResponse(message=f"Goal '{goal_title}' {action_desc}deleted successfully")
...
async def get_goal_progress(goal_id: str, db: Session = Depends(get_db)) -> dict:
    """
    ...
    **Database Implementation**: Calculates progress from linked projects with detailed breakdown.
    ...
    try:
        ...
        project_links = (
            db.query(GoalProject, Project)
            .join(Project, GoalProject.project_id == Project.id)
            .filter(GoalProject.goal_id == goal_id)
            .all()
        )
        ...
        final_progress = (
            current_progress / total_weight if total_weight > 0 else float(goal.progress_percentage)
        )

        # Get goal history (simplified - in real app would track changes)
        progress_history = [
            {"date": goal.created_at.date().isoformat(), "progress": 0.0},
            {"date": date.today().isoformat(), "progress": round(final_progress, 1)},
        ]
                ...
                milestones.append(
                    {
                        "title": "Halfway Point",
                        "target_date": (
                            goal.created_at.date() + (goal.target_date - goal.created_at.date()) / 2
                        ).isoformat(),
        ...
        raise HTTPException(status_code=500, detail=f"Error calculating goal progress: {str(e)}")
    ...
    **Database Implementation**: Retrieves subgoals with calculated progress.
    ...
    try:
        ...
        subgoals_data = QueryUtils.get_goals_with_progress(
            db=db, parent_goal_id=goal_id, page=page, size=size
        )
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving subgoals: {str(e)}")
...
async def get_goal_hierarchy(goal_id: str, db: Session = Depends(get_db)) -> dict:
    """
    ...
    try:
        ...
        def get_ancestors(goal_id: str) -> List[dict]:
            ...
            while current_goal and current_goal.parent_goal_id:
                ...
                if parent:
                    ...
                    project_links = (
                        db.query(GoalProject).filter(GoalProject.goal_id == parent.id).all()
                    )
                    if project_links:
                        total_weight = sum(float(link.weight) for link in project_links)
                        weighted_progress = 0.0
        ...
        def get_descendants(goal_id: str, level: int = 0) -> List[dict]:
            ...
            for subgoal in subgoals:
                ...
                project_links = (
                    db.query(GoalProject).filter(GoalProject.goal_id == subgoal.id).all()
                )
                if project_links:
                    total_weight = sum(float(link.weight) for link in project_links)
                    weighted_progress = 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving goal hierarchy: {str(e)}")
...
async def link_project_to_goal(
    ...
    weight: float = Query(
        1.0, ge=0.01, le=1.0, description="Weight for progress calculation (0.01-1.0)"
    ),
    db: Session = Depends(get_db),
) -> MessageResponse:
    """
    ...
    **Database Implementation**: Creates goal-project relationship.
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error linking project to goal: {str(e)}")
...
async def unlink_project_from_goal(
    ...
    **Database Implementation**: Removes goal-project relationship.
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error unlinking project from goal: {str(e)}")
