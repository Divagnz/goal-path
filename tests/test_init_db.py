...
class TestDatabaseInitializer:
    ...
    def test_create_goal_project_links(self, test_db_session):
        ...
        q2_launch = next(g for g in goals if g.title == "Q2 Product Launch")
        q2_links = test_db_session.query(GoalProject).filter(
            GoalProject.goal_id == q2_launch.id
        ).all()
    ...
    def test_task_hierarchy_integrity(self, test_db_session):
        ...
        child_tasks = test_db_session.query(Task).filter(
            Task.parent_task_id == design_system.id
        ).all()
    ...
    def test_project_task_relationships(self, test_db_session):
        ...
        for task in tasks:
            ...
            project = test_db_session.query(Project).filter(
