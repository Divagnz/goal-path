...
class TestQueryUtils:
    ...
    def test_get_projects_with_stats(self, test_db_session, db_helper):
        ...
        db_helper.create_test_task(test_db_session, project1.id, title="Task 2", status="in_progress")
    ...
    def test_get_projects_with_filters(self, test_db_session, db_helper):
        ...
        # Test status filter
        active_projects = QueryUtils.get_projects_with_stats(
            test_db_session, status="active"
        )
        ...
        # Test search filter
        search_results = QueryUtils.get_projects_with_stats(
            test_db_session, search="completed"
        )
...
class TestTransactionManager:
    ...
    def test_manual_commit(self, test_db_session, db_helper):
        ...
        saved_project = test_db_session.query(Project).filter(
            Project.name == "Manual Commit Project"
        ).first()
        assert saved_project is not None
    ...
    def test_manual_rollback(self, test_db_session, db_helper):
        ...
        saved_project = test_db_session.query(Project).filter(
            Project.name == "Manual Rollback Project"
        ).first()
        assert saved_project is None
