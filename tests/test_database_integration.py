...
class TestDatabaseIntegration:
    ...
    def test_database_session_lifecycle(self, test_db_session):
        ...
        # Verify it was saved
        saved_project = test_db_session.query(Project).filter(
            Project.name == "Session Test Project"
        ).first()
    ...
    def test_complex_hierarchy_queries(self, test_db_session, db_helper):
        ...
        epic = db_helper.create_test_task(
            test_db_session, project.id,
            title="Epic Task", task_type="epic"
        )
        
        story1 = db_helper.create_test_task(
            test_db_session, project.id,
            title="Story 1", task_type="story", parent_task_id=epic.id
        )
        
        story2 = db_helper.create_test_task(
            test_db_session, project.id,
            title="Story 2", task_type="story", parent_task_id=epic.id
        )
        
        subtask1 = db_helper.create_test_task(
            test_db_session, project.id,
            title="Subtask 1", task_type="subtask", parent_task_id=story1.id
        )
        
        subtask2 = db_helper.create_test_task(
            test_db_session, project.id,
            title="Subtask 2", task_type="subtask", parent_task_id=story1.id
        )
        
        # Test querying root tasks (no parent)
        root_tasks = test_db_session.query(Task).filter(
        ...
        epic_children = test_db_session.query(Task).filter(
            Task.parent_task_id == epic.id
        ).all()
        ...
        all_tasks = test_db_session.query(Task).filter(
            Task.project_id == project.id
        ).count()
    ...
    def test_goal_project_links_with_progress(self, test_db_session, db_helper):
        ...
        linked_projects = test_db_session.query(Project).join(GoalProject).filter(
        ...
        project_weights = test_db_session.query(Project.name, GoalProject.weight).join(
            GoalProject
        ).filter(GoalProject.goal_id == goal.id).all()
