        ...
        if projects_response.status_code == 200:
            ...
            if projects:
                ...
                htmx_error_response = requests.get(
                    "http://localhost:8001/projects/invalid-id", headers=htmx_headers
                )

                regular_404 = error_response.status_code == 404
                htmx_error_fragment = (
                    htmx_error_response.status_code == 200
                    and "not found" in htmx_error_response.text.lower()
                )

                print(f"   ✅ Regular request returns 404: {'✅' if regular_404 else '❌'}")
                print(
                    f"   ✅ HTMX request returns error fragment: {'✅' if htmx_error_fragment else '❌'}"
                )
                ...
                if projects_page.status_code == 200:
                    ...
                    nav_validations = {
                        "Has HTMX project links": f'hx-get="/projects/{project_id}"'
                        in projects_content,
                        "Links target main-content": 'hx-target="#main-content"'
                        in projects_content,
                        "Links push URL": 'hx-push-url="true"' in projects_content,
                        "Has View buttons": ">View<" in projects_content
                        or "View</a>" in projects_content,
                    }
