...
def test_complete_navigation_flow():
    ...
    try:
        ...
        if project_links:
            ...
            ), f"Back navigation failed: {back_response.status_code}"
            ...
            ), f"Direct access failed: {direct_response.status_code}"
            ...
            htmx_error_response = requests.get(
                f"{BASE_URL}/projects/nonexistent-id", headers=htmx_headers
            )
            assert (
                htmx_error_response.status_code == 200
            ), f"HTMX error handling failed: {htmx_error_response.status_code}"
...
def test_project_detail_features():
    ...
    try:
        ...
        features = {
            ...
            "Action Buttons": soup.find("button", string=lambda x: x and "Edit Project" in x)
            is not None,
            "Add Task Button": soup.find("button", string=lambda x: x and "Add Task" in x)
            is not None,
